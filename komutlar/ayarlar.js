...
exports.run = async (client, message, args) => {
  ...
  if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`<:red:603895302327500800> Bu komutu kullanabilmek için **Yönetici** iznine sahip olmalısın!`).then(msg => msg.delete(5000));
...
if (!a && a !== "destek" && a !== "kapat" && a !== "liste" && a !== "support" && a !== "off" && a !== "list") {
  ...
  .addField(dil.ayarlar.destekK, db.has(`destekK_${message.guild.id}`) ? `<:onay:603895302327500800>` + " <#"+db.fetch(`destekK_${message.guild.id}`)+">" : `<:red:553927970117517313> Ayarlanmamış`, true)
  .addField(dil.ayarlar.destekR, db.has(`destekR_${message.guild.id}`) ? `<:onay:603895302327500800>` + " `@"+message.guild.roles.get(db.fetch(`destekR_${message.guild.id}`)).name+"`" : `<:red:553927970117517313> Ayarlanmamış`, true)
  .setTimestamp()
  .setFooter(`${message.author.username} | Yetkili Tarafından Kontrol Ediliyor..`, message.author.avatarURL )
  message.channel.send(embed)
  return;
